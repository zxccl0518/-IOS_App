...
void decodeblock(unsigned char in[], char *clrstr, int count) {
	unsigned char out[4];
	out[0] = in[0] << 2 | in[1] >> 4;
	out[1] = in[1] << 4 | in[2] >> 2;
	out[2] = in[2] << 6 | in[3] >> 0;
	out[3] = '\0';
	memcpy(clrstr+count, out, 3);
}

int b64_decode(char *b64src, char *clrdst, int srcLen) {
	int c, phase, i;
	unsigned char in[4];
	char *p;
    int count = 0;
...
void b64_encode(char *clrstr, char *b64dst, int srcLen) {
	unsigned char in[3];
	int i, len = 0;
	int j = 0;
